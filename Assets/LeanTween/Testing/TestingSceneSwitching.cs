...
public class TestingSceneSwitching : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        LeanTween.scale(cube, new Vector3(3f, 3f, 3f), 0.1f).setDelay(0.1f)
            .setOnComplete(() => { tweenCompleteCnt++; });

        LeanTween.move(cube, new Vector3(3f, 3f, 3f), 0.1f).setOnComplete(() => { tweenCompleteCnt++; });

        LeanTween.delayedCall(cube, 0.1f, () => { tweenCompleteCnt++; });
