...
//		LeanTween.moveLocal(cube1, cube1.transform.localPosition+new Vector3(0f,1f,0f),1f).setEaseShake();
        ...
        LeanTween.sequence().append(LeanTween.scale(cube1, Vector3.one * 3f, .1f))
            .append(LeanTween.scale(cube1, Vector3.one * 5.6f, .2f));
        LeanTween.sequence().append(LeanTween.value(lightInScene.gameObject, f => lightInScene.range = f, 8, 12, .1f))
            .append(LeanTween.value(lightInScene.gameObject, f => lightInScene.range = f, 12, 8, .2f));
    }
